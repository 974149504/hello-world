...
public abstract class BaseMessage
{
    ...
    {
        init();
        message = new String(packet.getData(), packet.getOffset(), packet.getLength(), SipStack.encoding);
    }
     ...
     */
    public void setConnectionId(ConnectionIdentifier connId)
    {
        connectionId = connId;
    }
    ...
    public DialogIdentifier getDialogId()
    {
        ...
        String localTag, remoteTag;
        if (isRequest())
        {
            localTag = getToHeader().getTag();
            remoteTag = getFromHeader().getTag();
        }
        else
        {
            localTag = getFromHeader().getTag();
            remoteTag = getToHeader().getTag();
        }
        return new DialogIdentifier(callId, localTag, remoteTag);
    }
    ...
    {
        String callId = getCallIdHeader().getCallId();
        ViaHeader topVia = getViaHeader();
        String branch = null;
        if (topVia != null && topVia.hasBranch())
        {
        ...
        return new TransactionIdentifier(callId, seqn, method, branch);
    }

    /**
     * Gets the MethodIdentifier
     */
    public MethodIdentifier getUsernameMethodId()
    {
        String method = getCSeqHeader().getMethod();
        String fromUsername = getFromHeader().getNameAddress().getAddress().getUserName();
        return new MethodIdentifier(method, fromUsername);
    }
    ...
    {
        if (!isRequest())
        { // printWarning("getRequestLine(): called for no
            // request message\n",1);
            return null;
        }
    ...
    {
        if (!isResponse())
        { // printWarning("getStatusLine(): called for no
            // response message\n",1);
            return null;
        }
        SipParser par = new SipParser(message);
    ...
    protected void addHeaders(String str, boolean top)
    {
        ...
        String newMessage = head.concat(str);
        newMessage = newMessage.concat(tail);
        message = newMessage;
    }
    ...
    /**
     * Adds Header before the first header <i>referNamer</i> .
     * <p>
     * If there is no header of such type, it is added at top
     */
    public void addHeaderBefore(Header newHeader, String referNamer)
    {
        addHeadersBefore(newHeader.toString(), referNamer);
    }

    /**
     * Adds MultipleHeader(s) before the first header <i>referName</i> .
     * <p>
    ...
    {
        addHeadersBefore(mheader.toString(), referName);
    }

    /**
     * Adds Headers before the first header <i>referName</i> .
     * <p>
     * If there is no header of such type, they are added at top
     */
    protected void addHeadersBefore(String str, String referName)
    {
        if (!hasHeader(referName))
        {
            addHeaders(str, true);
        }
        else
    ...
    /**
     * Adds Header after the first header <i>referName</i> .
     * <p>
     * If there is no header of such type, it is added at bottom
     */
    public void addHeaderAfter(Header newHeader, String referName)
    {
        addHeadersAfter(newHeader.toString(), referName);
    }

    /**
     * Adds MultipleHeader(s) after the first header <i>referName</i> .
     * <p>
    ...
    {
        addHeadersAfter(mheader.toString(), referName);
    }

    /**
     * Adds Headers after the first header <i>referName</i> .
     * <p>
     * If there is no header of such type, they are added at bottom
     */
    protected void addHeadersAfter(String str, String referName)
    {
        if (!hasHeader(referName))
        {
     ...
     */
    public void setAcceptContactHeader(AcceptContactHeader h)
    {        // added by mandrajg
        setHeader(h);
    }
    ...
    public String getBody()
    {
        ...
        { // printWarning("No Content-Length header found for the
            // Body",3);
            len = message.length() - begin;
        }
        int end = begin + len;
        if (end > message.length())
        { // printWarning("Found a Message Body
    ...
    public void rfc2543RouteAdapt()
    {
        if (hasRouteHeader())
        {
            ...
            { // re-format the message according to the
                // RFC2543 Strict Route rule
                SipURL nextHop = (new RouteHeader(mrh.getTop())).getNameAddress().getAddress();
                SipURL recipient = getRequestLine().getAddress();
                mrh.removeTop();
                mrh.addBottom(new RouteHeader(new NameAddress(recipient)));
                setRoutes(mrh);
                setRequestLine(new RequestLine(getRequestLine().getMethod(), nextHop));
            }
        }
    }
    ...
    { // the message is formed
        ...
        SipURL nextHop = requestLine.getAddress();
        MultipleHeader mrh = getRoutes();
        SipURL target = (new RouteHeader(mrh.getBottom())).getNameAddress().getAddress();
        mrh.removeBottom();
        nextHop.addLr();
        mrh.addTop(new RouteHeader(new NameAddress(nextHop)));
        removeRoutes();
        addRoutes(mrh);
        setRequestLine(new RequestLine(requestLine.getMethod(), target));
    }
